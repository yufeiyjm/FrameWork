...
+ (void)getCalenderDataWithDate:(NSDate *)date selectedDate:(NSArray *)selectedDate  block:(CallBackBlock)block {
    ...
    for (NSInteger y = year; y < year + 2; y ++) {
        ...
        for (NSInteger i = month; i < 13; i++) {
            ...
            for (NSInteger j = 1; j < totalDay  +1; j ++) {
                ...
                subModel.day = j;
                NSString *strDay = [NSString stringWithFormat:@"%04zd-%02zd-%02zd",y,i,j];
                subModel.price = @"";
                for (NSInteger k = 0; k < selectedDate.count; k++) {
                    if ([selectedDate[k][@"dept_date"] isEqualToString:strDay]) {
                        subModel.price = [NSString stringWithFormat:@"%.0f", [selectedDate[k][@"lowestfare"] floatValue]];
                    }
                }
