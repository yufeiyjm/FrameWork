...
@interface InterviewEndListVC ()<UITableViewDelegate,UITableViewDataSource>
@property (weak, nonatomic) IBOutlet UITableView *tableView;
@property (strong, nonatomic)  NSMutableArray *arrData;//图片名数组

@end

@implementation InterviewEndListVC

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view from its nib.
    self.navigationItem.title = @"已面试";
    [self.tableView registerNib:[UINib nibWithNibName:@"InterviewEndListTableViewCell" bundle:nil] forCellReuseIdentifier:@"InterviewEndListTableViewCell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    static NSString *identifier = @"InterviewEndListTableViewCell";
    InterviewEndListTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    cell.selectionStyle =  UITableViewCellSelectionStyleNone;
    cell.reaseButton.tag = indexPath.row;
    [cell.reaseButton addTarget:self action:@selector(reaseButtonClick:) forControlEvents:UIControlEventTouchUpInside];
    return cell;
...
-(void)reaseButtonClick:(UIButton *)sender{
    ReaseInterviewVC *vc = [[ReaseInterviewVC alloc]init];
